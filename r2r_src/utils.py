...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            with open('data/R2R/R2R_%s.json' % split) as f:
...
def pad_instr_tokens(instr_tokens, maxlength=20):
    ...
    if len(instr_tokens) <= 1: #assert len(raw_instr_tokens) > 2
        return None

    if len(instr_tokens) > maxlength - 1: # -2 for [CLS] and [SEP]
        instr_tokens = instr_tokens[:(maxlength-1)]
    # instr_tokens = ['[CLS]'] + instr_tokens + ['[SEP]']
    instr_tokens = instr_tokens + ['[SEP]']
