...
def ensure_shared_grads(models, shared_models):
    for model, shared_model in zip(models, shared_models):
        for param, shared_param in zip(model.parameters(), shared_model.parameters()):
            if param.grad is None:
                shared_param._grad=None
                continue
            shared_param._grad = param.grad.cuda(0)
...
class SharedAdamW(optim.Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=1e-2, amsgrad=False):
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        super(SharedAdamW, self).__init__(params, defaults)
